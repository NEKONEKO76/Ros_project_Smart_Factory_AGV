...
def run(msg):
    ...
    with lock:
        ...
        if detect_step == 'line':
            ...
            color_center_x = center_x - 50
            color_center_y = center_y - 20
...
def move():
    ...
    position = {'red': 1, 'green': 1, 'blue': 1, 'None': -1}  # 色块对应位置横线数
            ...
            if set_visual == 'color':
                ...
                if not place_en:
                    ...
                    if time.time() >= place_delay:  # 延时停下来，把色块放到横线旁边
                        ...
                        target = ik.setPitchRanges((-0.24, 0.00, -0.00), -180, -180, 0)  # 机械臂移动到色块放置位置
                        if target:
                            servo_data = target[1]
                        ...
                        # 后退逻辑
                        set_velocity.publish(100, 270, 0)  # 设置小车后退速度
                        rospy.sleep(3.0)  # 调整时间，让小车回到初始位置
