...
# 更新日志
"""
在aproltage检测中，订阅了intelligen_palletizer_node中的服务，可以通过指令抓取指定id的tag
"""
...
def apriltag_Detect(img):
    ...
    # 从 ROS 参数服务器获取目标 ID（如果未设置，默认 2）
    target_id = rospy.get_param('/visual_processing/target_id', 99)
    # print(target_id)

    msg = Result()  # 创建一个对象，用于储存要publish的信息
    img_copy = img.copy()  # 复制输入图像，避免对原始图像进行修改。
    img_h, img_w = img.shape[:2]  # 获取图像的高度 img_h 和宽度 img_w。
    frame_resize = cv2.resize(img_copy, size_m,
                              interpolation=cv2.INTER_NEAREST)  # 将图像 img_copy 缩放到 size_m 尺寸，使用 INTER_NEAREST 进行最近邻插值。
    ...
    if len(detections) != 0:  # 判断是否有检测到 AprilTag。
        for i, detection in enumerate(detections):  # 遍历检测到的所有 AprilTag，i 是索引，detection 是检测到的每个 AprilTag 的详细信息。
            ...
            cv2.putText(img, str(tag_id), (object_center_x - 10, object_center_y + 10), cv2.FONT_HERSHEY_SIMPLEX, 1,
                        [0, 255, 255], 2)
            # 显示目前搜索的ID是多少？
            cv2.putText(img, str(target_id), (20, 20), cv2.FONT_HERSHEY_SIMPLEX, 1,
                        [0, 255, 255], 2)
...
# # 用于获取apriltag id的服务函数
# # 因为我们这里只需要知道这个ID是多少，而不需要改变这个ID，所以这个函数在这里非必要
# def set_target_id(req):
#     rospy.set_param('/visual_processing/target_id', req.data) #更新目标ID
#     rospy.loginfo(f"Updated target ID to {req.data}")
#     return SetBoolResponse(True, f"Updated target ID to {req.data}") # 返回成功信息
...
if __name__ == '__main__':
    ...
    # 用于获取apriltag id的服务函数
    # 这里不更改ID所以不需要
    # target_id_srv = rospy.Service('/intelligent_palletizer/set_target_id', SetBool, set_target_id)
